...
export default function PartnerCard({showFavoriteIcon}:{showFavoriteIcon?:boolean}) {
  return (
    <figure
    ...
    {
      showFavoriteIcon && (
        <button className='p-2 rounded-full text-green-500  bg-white absolute right-3 top-2'>
        <FaHeart/>
      </button>
      )
      ...
      <figcaption className='px-5 pb-5 pt-1'>
        ...
        <div className='flex gap-4 w-full justify-between mt-4'>
          <button
            className={`text-green-600 bg-white cursor-pointer ${styles.messageBtn} flex-1 flex justify-center gap-1 items-center border-1 border-green-600 rounded-lg  py-1 px-4`}
          >
